...
#' @export
getterm <- function(obj, term) UseMethod('getterm', obj)

#' @export
getterm.spotratecurve <- function(obj, term) {
  if (!is.numeric(term))
    as.term(bizdays(refdate(obj), term, calendar(obj)), units(obj))
  else
    as.term(term, units(obj))
}

#' @export
compound.spotratecurve <- function(obj, term)
  compound(obj[term], getterm(obj, term))
...
units=c('days', 'months', 'years')) {
  # here obj is rate.from
  units = match.arg(units)
  cf <- compound(rate.to, to)/compound(obj, from)
  comp <- compounding(obj)
  tf <- timefactor(daycount(obj), term=to-from, units=units)
  as.spotrate(rates(comp, cf, tf), comp, daycount(obj), calendar(obj))
}

#' @export
forwardrate.spotratecurve <- function(obj, from, to, forward=NULL) {
  if (missing(to)) {
    if (is.null(forward))
      stop("to or forward arguments must be provided.")
    else if (forward == 1 && (from == 1 || from == terms(obj)[1]))
      return(obj[1])
    else
      to <- from+forward
  }
  ...
  forwardrate(obj[from], getterm(obj, from), obj[to], getterm(obj, to), units(obj))
}
