...
plot.SpotRateCurve <- function(x, y, ...,
  ...
  plot_SpotRateCurve(x, y, ..., show_forward = show_forward,
...
plot_SpotRateCurve <- function(x, y, ...,
  ...
  y_ticks <- if (!is.na(y_ticks_n))
    seq(y_lower_lim, y_upper_lim, length.out = y_ticks_n) else
  ...
  axis(1, x_ticks, labels = x_ticks_lab,
       # family = family,
       cex.axis = 0.8)
  axis(2, y_ticks, labels = y_ticks_lab,
       # family = family,
       cex.axis = 0.8)
  ...
  if (use_interpolation) {
    ...
    idx <- seq_len(rng[2])
    interp_curve <- x[[idx]]
    lines(x = as.numeric(interp_curve@terms),
          y = as.numeric(interp_curve) * 100,
          lwd = 2, col = "#4f7f81", lty = 2)
    legend_[[length(legend_)+1]] <- list(lab = "Interpolation", col = "#4f7f81",
                                         pch = NA, lty = "dashed")
    if (show_forward) {
      rng <- range(x@terms)
      idx <- seq_len(rng[2])
      ...
      legend_[[length(legend_)+1]] <- list(lab = "Forward w/ Interpolation",
                                           col = "#fbb407",
                                           pch = NA, lty = "solid")
    }
  } else {
    ...
    if (show_forward) {
      ...
      legend_[[length(legend_)+1]] <- list(lab = "Forward",
                                           col = "#fbb407",
                                           pch = 19, lty = "solid")
    }
  }
  ...
  if (length(legend_) > 1) {
    ...
    legend(legend_location, inset = 0.02, legend = leg_labs, col = leg_cols,
