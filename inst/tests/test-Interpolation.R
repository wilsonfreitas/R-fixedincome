...
rates <- as.spotrate(rates, as.compounding('discrete'),
	as.daycount('actual/365'), Calendar(name='Actual'))

test_that("it should interpolate linearly the interest rate curve using functions", {
	curve <- as.spotratecurve(terms, rates, interp=linear)
	expect_true(curve[1] == 0.0719)
})

test_that("it should interpolate FlatForwardly the interest rate curve using functions", {
	curve <- as.spotratecurve(terms, rates, interp=flatforward)
	expect_equal(rates(curve[1]), 0.0719)
})

test_that("it should interpolate log-linearly the interest rate curve using functions", {
	curve <- as.spotratecurve(terms, rates, interp=log.linear)
	expect_equal(rates(curve[1]), 0.0719)
})

test_that("it should interpolate with natural spline the interest rate curve using functions", {
	curve <- as.spotratecurve(terms, rates, interp=natural.spline)
	expect_true(curve[1] == 0.0719)
})

test_that("it should interpolate with hermite spline the interest rate curve using functions", {
	curve <- as.spotratecurve(terms, rates, interp=hermite.spline)
	expect_true(curve[1] == 0.0719)
})

# test_that("it should interpolate FlatForwardly the interest rate curve using functions", {
# 	curve <- as.spotratecurve(terms, rates, interp=flatforward)
# 	expect_true(curve[21] == 0.065400692971856)
# })
